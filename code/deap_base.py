...
class Fitness(object):
    ...
    def setValues(self, L1_error_matrix):
        try:
            self.L1_error_matrix = L1_error_matrix
            self.calc_err()
            ...
            raise TypeError("Both weights and assigned values must be a "
                              ...
                              % (self.__class__, L1_error_matrix, type(L1_error_matrix),
                                 self.weights)).with_traceback(traceback)

    def calc_err(self):
        err_weighted = []
        for dt in self.datatypes:
            norm_err = self.L1_error_matrix[dt]/self.dat_norm_wts[dt]
            if(self.errNorm=='L2'):
                norm_err = norm_err**2
            elif(self.errNorm=='Lhalf'):
                norm_err = norm_err**0.5
            err = np.mean(norm_err)
    ...
    def setDatNormWts(self, dataweights):
        self.dat_norm_wts = dataweights
        if self.wvalues !=():
            self.calc_err()

    def setDataOptWts(self, dataoptwts):
        self.dat_opt_wts = dataoptwts
        if self.wvalues !=():
            self.calc_err(self)
        
    def delValues(self):
        self.wvalues = ()
        self.L1_error_matrix = ()
