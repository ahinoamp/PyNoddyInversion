...
#                       titleXOffset = 2,
#                       titleRotation = 180).z(zmax).x(xmax).scale(1.1)
...
legoPost.addScalarBar3D(title='P(fault)',
                        titleXOffset = 2,
                       titleRotation = 180).z(zmax).x(xmax+450)
...
vp2.show(legoPri, cornerptsP, 'hi1', at=0, elevation=-20, azimuth=0)
vp2.show(legoPost, cornerptsP, at=1, interactive=1, elevation=-20, azimuth=0)

##second
volPri = Volume(scalar_field_pri,spacing = spacing, origin=origin)
legoPri = volPri.legosurface(vmin=0, vmax=0.43, cmap="viridis")
#legoPri.addScalarBar3D(title='P(fault)',
#                       titleXOffset = 2,
#                       titleRotation = 180).z(zmax).x(xmax).scale(1.1)

volPost = Volume(scalar_field_post, spacing = spacing, origin=origin)
legoPost = volPost.legosurface(vmin=0.0, vmax=0.43, cmap="viridis")
#legoPost.addScalarBar3D(title='P(fault)',
#                        titleXOffset = 2,
#                       titleRotation = 180).z(zmax).x(xmax+450)

vp3 = Plotter(axes=dict(xtitle='Easting (m)',
#                        ytitle='Northing (m)', 
                        ztitle='Elevation (m)', yzGrid=False),
                        pos=(300, 400), N=2)
...
vp3.show(legoPri, 'hi1', at=0, elevation=-20, azimuth=0)
vp3.show(legoPost, at=1, interactive=1, elevation=-20, azimuth=0)
